...
regp.add_argument(
    ...
    dest="register",
    help=(
        "Ensure that all players are registered, or if a draw code is "
        "specified, or the argument passed multiple times with different "
        "draw codes, limit to these draws"
    ),
)
regp.add_argument(
    "--sleep",
...
regp.add_argument(
    ...
    action="store_true",
    dest="seedtourn",
    help="Adjust player grading points (seeding)",
)
regp.add_argument(
    "--extract",
    type=str,
    ...
    dest="extractr",
    help="Extract registrations file",
)
regp.add_argument(
    "--draw",
    "-d",
...
if not args.makedraws and (args.drawtypemap or args.drawdescmap):
    parser.error(
        "--draw-type and --draw-desc only make sense with --make",
        file=sys.stderr,
    )
...
with iSquashController(headless=args.headless, debug=args.debug) as c:
    ...
    if args.deldraws:
        ...
        def callback(draw_name):
            if data:
                print(f"  {data.draws[draw_name]!r} deleted", file=sys.stderr)
            else:
    ...
    if args.register:
        if args.register == 'ALL':
            ...
            players = [
                p for p in data.players.values() if p.draw.name in args.register
            ]
        ...
        c.go_fill_registrations(
            players, update=args.update, sleep=args.sleep, player_cb=callback
        )
        print(c, file=sys.stderr)
    ...
    if args.extractr:
        c.go_extract_registrations(args.extractr)
        print(c, file=sys.stderr)
    ...
    if args.extractd:
        c.go_extract_draws(args.extractd)
        print(c, file=sys.stderr)
